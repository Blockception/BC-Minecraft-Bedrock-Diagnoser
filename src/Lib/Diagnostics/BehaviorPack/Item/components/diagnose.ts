...
import { minecraft_get_item } from '../../../Minecraft';
...
const component_test: Record<string, ComponentCheck> = {
  ...
  "minecraft:entity_placer": (name, component, context, diagnoser) => {
    if (Array.isArray(component.dispense_on)) component.dispense_on.forEach((block: string) => {
  ...
  "minecraft:repairable": (name, component, context, diagnoser) => {
    if (Array.isArray(component.repair_items)) component.repair_items.forEach((repairEntry: any) => {
      if (Array.isArray(repairEntry.items)) repairEntry.items.forEach((item: string) => {
        // behaviorpack_item_diagnose(minecraft_get_item(item, diagnoser.document), diagnoser) TODO: diagonser.document doesn't exist; figure out how to get the document
      })
    })
  }
...
function deprecated_component(replacement?: string) {
  ...
  return component_error("This component is no longer supported" + str + '. You are recommended to use the latest format version.', 'behaviorpack.item.components.deprecated')
}